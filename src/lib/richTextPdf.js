  ...
  const drawListMarker = (type, mx, my, markerLh = lh) => {
    if (type === "arrow") {
      ...
      try {
        ...
        const arrowEndX = mx + Math.max(1.75, markerLh * 0.5);
        const headDX = Math.max(0.72, markerLh * 0.18);
        const headDY = Math.max(0.48, markerLh * 0.14);
        const stemH = Math.max(0.24, markerLh * 0.085);
